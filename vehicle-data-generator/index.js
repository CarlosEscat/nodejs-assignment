...
const readOutLoud = vehicleName => {
  ...
  // fs.createReadStream will read a file in pieces with an specified size, while fs.readFileSync and fs.readFileAsync
  // buffers the whole file into the memory.
        ...
        new Writable({
          ...
          write(obj, enc, cb) {
            ...
            setTimeout(() => {
              ...
              if (i % 100 === 0)
                console.log(
                  `vehicle ${vehicleName} sent have sent ${i} messages`
                );
...
readOutLoud("test-bus-1").once("finish", () => {
  console.log(
    "henk is on the last stop and he is taking a cigarrete while waiting for his next trip"
  );
});
